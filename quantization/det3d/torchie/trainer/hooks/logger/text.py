        ...
        for idx, task_class_names in enumerate(class_names):
            ...
            for name, val in log_dict.items():
                ...
                if isinstance(val, (list, tuple)):
                    if idx < len(val):
                        log_items.append("{}: {}".format(name, self._convert_to_precision4(val[idx])))
                    else:
                        continue  # elkerüli a túlindexelést
                else:
                    # scalar érték (nem lista)
                    log_items.append("{}: {}".format(name, self._convert_to_precision4(val)))
