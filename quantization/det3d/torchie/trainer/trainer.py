        ...
        while self.epoch < max_epochs:
            for i, flow in enumerate(workflow):
                if self.epoch >= self._fade_epochs:
                    try:
                        data_loaders[i].dataset.pipeline.transforms[2].db_sampler = None # Itt módosítottam annyit, hogy 
...
class CustomTrainer(Trainer):
    ...
    def __init__(self, test_cfg, model, optimizer=None, lr_scheduler=None,
                 work_dir=None, log_level=logging.INFO, logger=None, **kwargs):
        
        super().__init__(
            model=model,
            batch_processor=lambda *args, **kwargs: None,  # optional, most a saját inline függvényedet használod
            optimizer=optimizer,
    ...
    def batch_processor_inline(self, model, data, train_mode, **kwargs):
        ...
        if train_mode:
            out = model(data['bev'])
            losses = model.loss(example, out, self.test_cfg)
            self.call_hook("after_forward")
            loss, log_vars = parse_second_losses(losses)
            del losses
