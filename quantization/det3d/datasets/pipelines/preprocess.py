...
class AssignLabel(object):
    def __init__(self, **kwargs):
        ...
        # self.out_size_factor = assigner_cfg.out_size_factor
        self.tasks = assigner_cfg['target_assigner']['tasks'] #assigner_cfg.target_assigner.tasks
        self.gaussian_overlap = assigner_cfg['gaussian_overlap'] #assigner_cfg.gaussian_overlap
        self._max_objs = assigner_cfg['max_objs'] #assigner_cfg.max_objs
        self._min_radius = assigner_cfg['min_radius'] #assigner_cfg.min_radius
        self.pc_range = assigner_cfg['pc_range'] #assigner_cfg.pc_range
        self.voxel_size = assigner_cfg['voxel_size'] #assigner_cfg.voxel_size
        self.pc_range = np.array(assigner_cfg['pc_range'], dtype=np.float32) #np.array(assigner_cfg.pc_range, dtype=np.float32)
        self.voxel_size = np.array(assigner_cfg['voxel_size'], dtype=np.float32) #np.array(assigner_cfg.voxel_size, dtype=np.float32)
        self.grid_size = np.round((self.pc_range[3:] - self.pc_range[:3]) / self.voxel_size).astype(np.int64)
    ...
    def __call__(self, res, info):
        ...
        if "gt_classes" not in res["lidar"]["annotations"]:
            annos = res["lidar"]["annotations"]
            # neveket és dobozokat az elvárt formába konvertáljuk
            res["lidar"]["annotations"] = {
                "gt_boxes": annos["boxes"],
                "gt_names": annos["names"],
                "gt_boxes_token": annos["tokens"],
                "gt_boxes_velocity": annos["velocities"],
                "gt_classes": np.array([
                    next(
                        (i  for i, name in enumerate(
                            sum(class_names_by_task, [])
                        ) if name == n),
                        -1
                    )
                    for n in annos["names"]
                ], dtype=np.int32),
            }
        ...
        if res["mode"] == "train":
            ...
            for idx, task in enumerate(self.tasks):
                cur_feat_map_size = feature_map_size // task['stride']
                ...
                for k in range(num_objs):
                    ...
                    w, l = w / voxel_size[0] / task['stride'], l / voxel_size[1] / task['stride']
                    if w > 0 and l > 0:
                        ...
                        coor_x, coor_y = (x - pc_range[0]) / voxel_size[0] / task['stride'], \
                                         (y - pc_range[1]) / voxel_size[1] / task['stride']
                        ...
                        if not (0 <= ct_int[0] < cur_feat_map_size[0] and 0 <= ct_int[1] < cur_feat_map_size[1]):
                            continue
                        ...
                        cat[new_idx] = cls_id
                        ind[new_idx] = y * cur_feat_map_size[0] + x
                        mask[new_idx] = 1
